...
export default function Chart4() {
    ...
    const getValues:string = localStorage.getItem("copper-concentration") || '';
    if(getValues){
    ...
    const drawChart = (data: CopperConcentration[]) => {
        const result = [];
        const dd: number[] = []
    ...
    const handleDrawChart = async () => {
        localStorage.setItem("copper-concentration",JSON.stringify({hip: hip}))

        const res = await api.get("/copperConcentration");
        if(res?.status === 200 && res?.data){
            const rows:CopperConcentration[] = []

            res.data.map((e:CopperConcentration) => {
                rows.push({
                    id: e._id,
            ...
            const fl: CopperConcentration[] = rows.filter(e => e.copCon && e.hipId === hip)
